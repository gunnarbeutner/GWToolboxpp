...
namespace {
    ...
    bool suppress_hero_behavior_messages = false;
    clock_t suppress_start = 0;
    constexpr clock_t SUPPRESS_DURATION_MS = 1000;
...
namespace {
    ...
    GuiUtils::EncString* merc_enc_names[8] = {};
                    ...
                    for (size_t i = 0; i < heroes.size(); i++) {
                        if (heroes[i].owner_player_id == me->login_number &&
                            ...
                            if (merc_agent_ids[m] != heroes[i].agent_id) {
                                ...
                                if (enc) {
                                    if (!merc_enc_names[m])
                                        merc_enc_names[m] = new GuiUtils::EncString();
...
void HotkeyHeroBehavior::Execute()
{
    ...
    GW::GameThread::Enqueue([&] {
        ...
        const auto* me = GW::Agents::GetControlledCharacter();
        if (!me) return;
        suppress_hero_behavior_messages = true;
        suppress_start = TIMER_INIT();
        for (size_t i = 0; i < heroes.size(); i++) {
...
bool HotkeyHeroBehavior::ShouldSuppressChatMessage(const wchar_t* message, const uint32_t channel)
{
    if (!message || !suppress_hero_behavior_messages) return false;
    if (channel != GW::Chat::Channel::CHANNEL_GROUP) return false;
    if (message[0] != 0x076B) return false;
    if (TIMER_DIFF(suppress_start) > SUPPRESS_DURATION_MS) {
        suppress_hero_behavior_messages = false;
        return false;
    }
    // Check if message contains a hero behavior string ID (Fight/Guard/Avoid)
    for (const wchar_t* p = message; *p; p++) {
        if (p[0] == 0x8101 && (p[1] == 0x5E27 || p[1] == 0x5E28 || p[1] == 0x5E29)) {
            return true;
        }
    }
    return false;
